...
function Order() {
  const {
    ...
    unit,
    setUnit,
  } = useTopic(); // Get topic from context
  ...
  const getCurrencyDetails = (countryCode: string): CurrencyData => {
    switch (countryCode) {
      ...
      case "GB":
        return { symbol: "£", ppp: 20.0, unit: "gbp" };
      case "NL":
      case "NO":
        return { symbol: "€", ppp: 23.83, unit: "eur" };
      case "NZ":
  ...
  if (loading)
    return (
      <div className="h-screen  text-2xl font-semibold flex justify-center items-center gap-2">
      ...
      <div className="grid lg:grid-cols-3 grid-cols-1 gap-4 p-4 ">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="">
            ...
            <TabsContent value="instruction">
              <PaperInstruction
                ...
                onPrevious={handlePreviousToType}
              />
            </TabsContent>
            <TabsContent value="details">
              <ContactDetails
                onPrevious={handlePreviousToInstruction}
        ...
        <div>
          <div className="py-5 px-2 dark:bg-gray-900 bg-zinc-900 text-white rounded-2xl overflow-hidden">
            <div className="text-xl md:text-3xl text-center py-3 font-bold ">
              Order Summary
            </div>
            ...
            <div className="flex justify-between items-center px-3 py-3 font-semibold">
              ...
              <div className="bg-green-500 text-sm rounded-lg p-1 text-white">
                Free
              </div>
            </div>
            <div className="flex justify-between items-center px-3 py-3 font-semibold">
              <div>Discount</div>
              <div className="bg-orange-500 text-sm rounded-lg p-1 text-white border-2 border-dotted">
                45% Applied <span className="font-bold">(FLAT45OFF)</span>
              </div>
            </div>
            <div className="flex justify-center py-3 px-3">
              <Image
                src={"/imgs/card-dissertationist.webp"}
                width={400}
                height={200}
                alt="card"
              ></Image>
